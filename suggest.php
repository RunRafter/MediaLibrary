...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // if data from form has been submitted
  ...
  // trim makes validating simpler so that "(blank space)" is also not valid
  
  // checks if required fields are filled in
  if ($name == "" || $email == "" || $category == "" || $title == "") {
    $error_message = "Please fill in the required fields: Name, Email, Category, and Title.";
  }
  if (!isset($error_message) && $_POST["address"] != "") { // checks if spam honeypot has been left blank; lower priority than first error
  ...
  // in event that email is not valid, display error; lower priority than first error
  if (!isset($error_message) && !$mail->ValidateAddress($email)) {
    $error_message = "Invalid Email Address";
  }
  
  // below runs if none of above errors occur
  if (!isset($error_message)) {
    // create a variable to hold all the values from the form - to be emailed out
    $email_body = "";
    ...
    $mail->addAddress('treehouse@localhost', 'Tony');     // Add a recipient
...
?>
    <form method="post" action="suggest.php">
      ...
      <tr>
        ...
        <td><textarea id="details" name="details"><?php if (isset($details)) { echo $_POST["details"]; } ?> 
          </textarea></td>
        <!-- if used $_POST["details"] instead, MUST escape output using htmlspecialchars($_POST["details"]) -->
      </tr>
...
<!-- RATIONALE
suggest.php now handles three functions
1. displaying the form (skips over first if, and thank you if)
2. handling submission -> submit back to same file because of action attribute. runs first if statement since form was posted. Redirects to sage page again  but with query string added
3. display thank you msg since status=thanks is in query string now -->
